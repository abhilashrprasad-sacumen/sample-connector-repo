...
class QualysAWSConnector:
    ...
    def _parse_pagination(self, data: Dict[str, Any]) -> PaginationInfo:
        ...
        return PaginationInfo(
            ...
            page_size=pageable.get("size", pageable.get("pageSize", 0)),
            # Using 'total_pages' but API returns 'totalPages'
            total_pages=data.get("totalPages", data.get("totalPages", 0)),
            # Using 'total_elements' but API returns 'totalElements'
            total_elements=data.get("totalElements", data.get("totalElements", 0)),
            # Using 'number_of_elements' but API returns 'numberOfElements'
            number_of_elements=data.get("numberOfElements", data.get("numberOfElements", 0))
        )
